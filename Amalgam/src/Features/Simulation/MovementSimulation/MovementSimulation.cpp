...
void CMovementSimulation::Store()
{
	for (auto pEntity : H::Entities.GetGroup(EntityEnum::PlayerAll))
	{
		...
		if (pLastRecord)
		{
			float flPosDelta = tCurRecord.m_vOrigin.DistTo(pLastRecord->m_vOrigin);
			if (flPosDelta > 500.f)
			{
				vRecords.clear();
				continue;
			...
			SDK::TraceHull(pLastRecord->m_vOrigin, pLastRecord->m_vOrigin + pLastRecord->m_vVelocity * TICK_INTERVAL,
				pPlayer->m_vecMins() + 0.125f, pPlayer->m_vecMaxs() - 0.125f, pPlayer->SolidMask(), &filter, &trace);
...
bool CMovementSimulation::SetupMoveData(MoveStorage& tStorage)
{
	...
	if (!tStorage.m_MoveData.m_vecVelocity.To2D().IsZero())
	{
		...
		else
		{
			...
			{
				Vec3 vVelocity = tStorage.m_MoveData.m_vecVelocity;
				float flSpeed = vVelocity.Length2D();
				float flVelYaw = Math::VectorAngles(vVelocity).y;

				if (flSpeed > 10.f)
				{
					if (!tStorage.m_bDirectMove && fabsf(tStorage.m_flAverageYaw) > 0.5f)
					{
						tStorage.m_MoveData.m_vecViewAngles = { 0.f, flVelYaw, 0.f };
					}
					else
					{
						Vec3 vEyeAngles = H::Entities.GetEyeAngles(iIndex);
						float flDiff = fabsf(Math::NormalizeAngle(vEyeAngles.y - flVelYaw));

						if (flDiff < 60.f)
							tStorage.m_MoveData.m_vecViewAngles = vEyeAngles;
						else
							tStorage.m_MoveData.m_vecViewAngles = { 0.f, flVelYaw, 0.f };
					}
				}
...
static inline bool GetYawDifference(
	MoveData& tRecord1, MoveData& tRecord2, bool bStart, float* pYaw,
...
void CMovementSimulation::GetAverageYaw(MoveStorage& tStorage, int iSamples)
{
	...
	std::vector<std::pair<float, float>> vWeightedYaws;
	...
	for (; i < iSamples; i++)
	{
		...
		float flYaw = 0.f;
		bool bResult = GetYawDifference(tRecord1, tRecord2, !iTicks, &flYaw, flStraightFuzzyValue,
			iMaxChanges, iMaxChangeTime, flMaxSpeed,
			tStorage.m_iYawChanges, tStorage.m_iYawStart,
			tStorage.m_iYawSign, tStorage.m_bYawZero);
		...
		// bbbbbb
		float flWeight = expf(-0.12f * float(i - 1));

		float flSpeedRatio = std::clamp(tRecord1.m_vVelocity.Length2D() / flMaxSpeed, 0.f, 1.f);
		float flConfidence = flSpeedRatio * flSpeedRatio;
		flWeight *= flConfidence;

		vWeightedYaws.push_back({ flYaw, flWeight });
	...
	if (i <= size_t(iMinimumStrafes + iSkips))
		return;
	...
	float flTimeSpan = vRecords[0].m_flSimTime - vRecords[i - 1].m_flSimTime;
	if (flTimeSpan < 0.22f) // 12312312321312
		return;

	float flWeightedSum = 0.f, flTotalWeight = 0.f;
	for (const auto& [yaw, weight] : vWeightedYaws)
	{
		flWeightedSum += yaw * weight;
		flTotalWeight += weight;
	}

	if (flTotalWeight < 0.01f)
		return;

	float flAverageYaw = flWeightedSum / flTotalWeight;

	if (fabsf(flAverageYaw) < 0.36f)
		return;

	// Distance-based prediction scaling for non-local players
	...
	SDK::Output("MovementSimulation", std::format("flAverageYaw calculated to {} from {} samples over {:.2f}s {}",
		flAverageYaw, vWeightedYaws.size(), flTimeSpan, pPlayer->entindex() == I::EngineClient->GetLocalPlayer() ? "(local)" : "").c_str(),
...
bool CMovementSimulation::SetDuck(MoveStorage& tStorage, bool bDuck)
{
...
void CMovementSimulation::RunTick(MoveStorage& tStorage, bool bPath, std::function<void(CMoveData&)>* pCallback)
{
	...
	if (tStorage.m_flAverageYaw)
	{
		...
		if (tStorage.m_flSmoothedYaw == 0.f)
			tStorage.m_flSmoothedYaw = tStorage.m_flAverageYaw;
		else
			tStorage.m_flSmoothedYaw = tStorage.m_flSmoothedYaw * (1.f - flAlpha) + tStorage.m_flAverageYaw * flAlpha;

		float flMult = 1.f;
		if (!tStorage.m_bDirectMove && !tStorage.m_pPlayer->InCond(TF_COND_SHIELD_CHARGE))
		{
			flCorrection = 90.f * sign(tStorage.m_flSmoothedYaw);
			if (Vars::Aimbot::Projectile::MovesimFrictionFlags.Value & Vars::Aimbot::Projectile::MovesimFrictionFlagsEnum::RunReduce)
				flMult = GetFrictionScale(tStorage.m_MoveData.m_vecVelocity.Length2D(),
					tStorage.m_flSmoothedYaw,
					tStorage.m_MoveData.m_vecVelocity.z + GetGravity() * TICK_INTERVAL);
		}

		tStorage.m_MoveData.m_vecViewAngles.y += tStorage.m_flSmoothedYaw * flMult + flCorrection;
	}
