    ...
    CCommand& args, cmd_source_t source)
{
    ...
    if (args.ArgC())
    {
        ...
        case FNV1A::Hash32Const("say_team"):
        {
            ...
            s_sCmdString = s_sCmdString.substr(args.m_nArgv0Size);
            while (!s_sCmdString.empty() && isspace(s_sCmdString[0]))
                s_sCmdString.erase(0, 1);

            // Strip quotes if they surround the whole message
            if (s_sCmdString.size() >= 2 && s_sCmdString.front() == '"' && s_sCmdString.back() == '"')
            {
                s_sCmdString.pop_back();
                s_sCmdString.erase(0, 1);
            }

            if (F::ChatTranslator.SendOutgoing(s_sCmdString.c_str()))
                return;
